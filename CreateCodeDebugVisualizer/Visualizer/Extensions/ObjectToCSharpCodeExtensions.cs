...
namespace Visualizer.Extensions
{
	public static class ObjectToCSharpCodeExtensions
	{
		public static string CreateCode(this object obj)
		{
			...
			{
				var genericTypeName = obj.IsOfType<Array>() ? obj.GetType().GetElementType() : obj.GetType().GetGenericArguments()[0];
		...
		private static string CreateCode(this object obj, StringBuilder sb)
		{
			...
			foreach (var propertyInfo in type.GetProperties())
			{
				...
				{
					sb.AppendLine(string.Format(@"{0}.{1} = ""{2}"";", instanceName, propertyInfo.Name, propertyInfo.GetValue(obj)));
				}
				...
				{
					sb.AppendLine(string.Format("{0}.{1} = {2};", instanceName, propertyInfo.Name, propertyInfo.GetValue(obj).ToString().ToLower()));
				}
					...
					))
				{
					...
					if (list != null)
					{
						...
						sb.AppendLine(string.Format("var {0} = new List<{1}>();", propertyInfo.Name.ToCamelCase(), genericTypeName));
						...
						sb.AppendLine(string.Format("{0}.{1} = {2}{3};", instanceName, propertyInfo.Name, propertyInfo.Name.ToCamelCase(),
															 isArray ? ".ToArray()" : ""));
					}
				}
